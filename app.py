...
filepath = "/root/fisco/nodes/127.0.0.1/node0/log_record/"
# filepath = "/root/RecorderfileFlask-fisco_show/log_record/"
...
# # 读取csv文件，去掉重复表头行
# def read_csv_without_duplicates(path, max_rows=1000):
#     # 暂存表头
#     header = pd.read_csv(path, nrows=0).columns.tolist()
#     # 读取CSV文件，忽略表头
#     df = pd.read_csv(path, header=0, nrows=max_rows)
#     # 删除所有重复行
...
def read_csv_without_duplicates(path, max_rows=30000):
    # 预读取表头
    with open(path, 'r', encoding='utf-8') as f:
        header = f.readline().strip().split(',')
    # 用 deque 保存最后 max_rows 行
    last_rows = deque(maxlen=max_rows)
    # 块读取并缓存每行数据
    for chunk in pd.read_csv(path, chunksize=10000, skiprows=1, header=None, names=header):
        for row in chunk.itertuples(index=False, name=None):
            last_rows.append(row)
    # 将 deque 转为 DataFrame
    df = pd.DataFrame(last_rows, columns=header)
    # 删除所有重复行
    df = df.drop_duplicates(keep=False)
    return df
...
def get_peer_message_throughput():
    ...
    # 格式化时间戳
    df['measure_time'] = df['measure_time'].apply(format_time)
    ...
    total = len(received_df)
    zoom_range = 40 / total * 100
    ...
    line1 = (
        ...
        .set_global_opts(title_opts=opts.TitleOpts(title="节点收发消息总量-接收"),
                         ...
                         datazoom_opts=[
                             opts.DataZoomOpts(range_start=95-zoom_range, range_end=95, pos_bottom="50%",type_="inside",xaxis_index=0),
                             opts.DataZoomOpts(range_start=100-zoom_range, range_end=100, pos_bottom="0%",type_="slider",xaxis_index=1),
                         ],
                         xaxis_opts=opts.AxisOpts(name="测量时刻"),
                         yaxis_opts=opts.AxisOpts(name="消息大小（字节）"),
...
def get_net_p2p_transmission_latency():
    ...
    duration_list_1000 = [float(f"{x * 1000:.6f}") for x in duration_list]

    total = len(duration_list)
    zoom_range = 30 / total * 100
    # 获取duration_list中的最大值最小值
    min_value = float(min(duration_list_1000))
    max_value = float(max(duration_list_1000))
    bar = (
        Bar()
        .add_xaxis(shortened_receive_id_list)
        .add_yaxis(series_name="平均传输时间", y_axis=duration_list_1000)
        .set_global_opts(title_opts=opts.TitleOpts(title="从" + shorten_id(send_id) + "发出消息计算结果"),
                         ...
                         datazoom_opts=[
                             opts.DataZoomOpts(range_start=100-zoom_range, range_end=100),
                         ],
                         xaxis_opts=opts.AxisOpts(name="接收消息节点ID"),
                         yaxis_opts=opts.AxisOpts(name="平均传输时间/ms"),
                         legend_opts=opts.LegendOpts(pos_left="center"),
                         )
...
def get_db_state_read_rate():
    ...
    bar = (
        ...
        .set_global_opts(title_opts=opts.TitleOpts(title="数据层读取速率"),
                         toolbox_opts=opts.ToolboxOpts(),
                         visualmap_opts=opts.VisualMapOpts(
                             min_=min_value,
                             max_=max_value,
    ...
    bar_hist = construct_cdf_chart(new_df['value'], 3, "数据层读取耗时")
...
def get_tx_queue_delay():
    ...
    if len(data) != 0:
        ...
        max_value = float(max(data, key=lambda x: x['value'])['value'])
    total = len(data)
    zoom_range = 55 / total * 100
    bar = (
        Bar()
        .add_xaxis(df["start_time"].tolist())
        .add_yaxis(series_name="交易排队延迟", y_axis=data)
        .set_global_opts(title_opts=opts.TitleOpts(title="交易排队延迟"),
                         toolbox_opts=opts.ToolboxOpts(),
                         visualmap_opts=opts.VisualMapOpts(
                             min_=min_value,
                             max_=max_value,
                         ...
                         xaxis_opts=opts.AxisOpts(name="进入交易池时刻"),
                         yaxis_opts=opts.AxisOpts(name="交易排队延迟/s"),
                         tooltip_opts=opts.TooltipOpts(
                             formatter=JsCode(
                                 """
                                 function (params) {
    ...
    bar_hist = construct_cdf_chart(df['value'], 10, "交易排队延迟")
...
def get_transaction_pool_input_throughput():
    ...
    txpool_input_throughput = round(txpool_input_throughput, 6)
    # 把1修改为local 2修改为rpc
    ...
    JsStr = "['开始时间: " + start_time + "','结束时间: " + end_time + "','总记录时间: " + str(
        ...
        txpool_input_throughput) + 'Tx/s' + "'].join('\\n')"
...
def get_tx_in_block_tps():
    ...
    # 保留6位
    df['value'] = df['value'].apply(lambda x: round(float(x), 4))
    # 将新的DataFrame转换为字典列表
    data = df.to_dict('records')
    # 获取value中的最大值最小值
    min_value, max_value = 0, 100
...
def get_block_tx_conflict_rate():
    # 读取csv文件
    df = read_csv_without_duplicates(filepath + '/block_tx_conflict_rate.csv')
    if len(df) <= 0:
        return render_template('draw.html', chart="<h2>当前文件尚无数据</h2>")
    # 按block_height分组
    ...
    for name, group in grouped:
        ...
        # 计算conflict_rate
        conflict_rate = float(cft_cnt) / float(block_tx_count)
        # 将结果添加到新的DataFrame中
...
def get_block_validation_efficiency():
    # 读取csv文件
    df = read_csv_without_duplicates(filepath + '/block_validation_efficiency.csv')
    if len(df) <= 0:
        return render_template('draw.html', chart="<h2>当前文件尚无数据</h2>")
    # 计算时间差
    df['duration'] = df.apply(lambda row: calculate_duration(row['end_time'], row['start_time']), axis=1)
    df = df[df['duration'] >= 0]
    # 计算区块验证效率 = 块内交易数 / 验证耗时
    df['block_tx_count'] = df['block_tx_count'].astype(float)
    df['duration'] = df['duration'].astype(float)
    df['valid_efficiency'] = (df['block_tx_count'] / df['duration']).round(2)
...
def format_time(time_str):
    formatted_time = "%Y-%m-%d %H:%M:%S.%f"
    dt = datetime.strptime(time_str, formatted_time)
